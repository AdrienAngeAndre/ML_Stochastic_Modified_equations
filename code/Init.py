...
def create_opt(models,func):
    ...
    optimizer = torch.optim.AdamW(all_parameters, lr=1e-4,weight_decay=1e-9) 
...
def init_Linear(nb_point,dim):
    lambd, mu = param("Linear")
    y0 = torch.rand(nb_point, dim)*2
...
def init_Pend(nb_point,h_fixé,Tube,Rand):
    ...
    for i in range(0,nb_point):
        ...
        x.append(Meth_Non_Mod(y0[i],h[i]/10,h[i],1000000,"MidPoint",Rand,"Pendule")[0])
